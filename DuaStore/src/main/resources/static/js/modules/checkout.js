...
function saveAddressFromModal() {
    ...
    if (!document.getElementById('modalTinhThanh').value) { DuaStore.toast.warning('Vui lòng nhập tỉnh/thành'); return; }
    if (!document.getElementById('modalQuanHuyen').value) { DuaStore.toast.warning('Vui lòng nhập quận/huyện'); return; }
    if (!document.getElementById('modalPhuongXa').value) { DuaStore.toast.warning('Vui lòng nhập phường/xã'); return; }

    var formData = new URLSearchParams();
    if (editingAddressId) formData.append('id', editingAddressId);
    ...
    .catch(function() { DuaStore.toast.error('Lỗi kết nối'); });
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('keydown', function(e) {
        if (e.target.id === 'modalDiaChiCuThe' && e.key === 'Enter') { e.preventDefault(); modalSearchMap(); }
    });
    ...
    document.getElementById('qrConfirmBtn')?.addEventListener('click', function() {
        ...
        fetch('/checkout/api/create', { method: 'POST', body: new URLSearchParams(formData) })
            ...
            .catch(function() { DuaStore.toast.error('Lỗi kết nối'); btn.disabled = false; btn.textContent = 'Đã thanh toán'; });
    });
    ...
    document.querySelectorAll('input[name="phuongThucGiaoHang"]').forEach(function(el) { el.addEventListener('change', function() { updateShipFee(); }); });
    document.querySelectorAll('input[name="phuongThucTT"]').forEach(function(el) {
        el.addEventListener('change', function() { document.getElementById('ckInfo').style.display = el.value === 'CHUYEN_KHOAN' && el.checked ? 'block' : 'none'; });
    });
